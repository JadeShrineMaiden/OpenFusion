...
static void becomeNpcCommand(std::string full, std::vector<std::string>& args, CNSocket* sock) {
    ...
    int range = 500;
    if (args.size() > 2)
        range = std::strtol(args[2].c_str(), &rest, 10);
...
static void becomePlayerCommand(std::string full, std::vector<std::string>& args, CNSocket* sock) {
    Player* plr = PlayerManager::getPlayer(sock);
    NPCManager::destroyNPC(plr->followerNPC);
    plr->followerNPC = 0;
    plr->vanished = false;
}
