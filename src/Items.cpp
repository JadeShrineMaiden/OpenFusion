...
std::map<std::pair<int32_t, int32_t>, Bank*> Items::TempBank;
...
static void itemMoveHandler(CNSocket* sock, CNPacketData* data) {
    ...
    std::pair<int32_t, int32_t> key = std::make_pair(plr->iID, plr->activeBank);
    ...
    case SlotType::BANK:
        ...
        fromItem = &TempBank[key]->bankSlot[itemmove->iFromSlotNum];
        break;
    default:
    ...
    case SlotType::BANK:
        ...
        toItem = &TempBank[key]->bankSlot[itemmove->iToSlotNum];
        break;
    default:
...
static void itemUseHandler(CNSocket* sock, CNPacketData* data) {
    ...
    if (gumball.iID>=119 && gumball.iID<=121) {
        ...
        resp.iSkillID = 144;
        if (resp.eST != 1001 && resp.eST != 1002) {
            gumball.iOpt -= 1;
            if (gumball.iOpt == 0)
                gumball = {};
        }
...
static void itemBankOpenHandler(CNSocket* sock, CNPacketData* data) {
    ...
    for (int i = 0; i < ABANK_COUNT; i++) {
        resp.aBank[i] = TempBank[key]->bankSlot[i];
    }
